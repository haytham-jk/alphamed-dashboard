...
export default function TrainingPage({ canEdit }) {
  ...
  return <div className="space-y-5">
    <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-sm text-blue-400">Customer development</p><h1 className="text-3xl font-semibold">Training Records</h1></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setRefreshKey((value) => value + 1)} disabled={refreshing} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-700 px-4 py-2 transition-all duration-200 hover:border-blue-600 hover:bg-slate-800 hover:text-white active:scale-[0.98] disabled:opacity-60"><RefreshCw size={17}/>{refreshing ? "Refreshing..." : "Refresh"}</button>{canEdit && <button type="button" onClick={() => navigate("/training/new")} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2 font-medium text-white transition-all duration-200 hover:bg-blue-500 hover:shadow-lg hover:shadow-blue-950/40 active:scale-[0.98]"><Plus size={18}/>New training</button>}</div></header>
    ...
    <p className="flex items-center gap-2 text-sm text-slate-400" aria-live="polite"><GraduationCap size={17}/>Showing <strong className="text-slate-200">{result.rows.length}</strong> of <strong className="text-slate-200">{result.filteredCount}</strong> matching records, <strong className="text-slate-200">{result.totalCount}</strong> total</p>
