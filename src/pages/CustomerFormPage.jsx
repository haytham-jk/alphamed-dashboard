...
export default function CustomerFormPage() {
  const { customerId } = useParams(); const navigate = useNavigate(); const location = useLocation(); const editing = Boolean(customerId); const formRef = useRef(null);
  const customersReturnTo = safeReturnPath(location.state?.customersReturnTo, "/customers", "/customers");
  const customerReturnState = buildFocusState("focusCustomerId", location.state?.focusCustomerId || customerId);
  const [values, setValues] = useState(initialValues); const [contactDraft, setContactDraft] = useState(null); const [editingContactKey, setEditingContactKey] = useState(""); const [similar, setSimilar] = useState([]); const [duplicateCheckKey, setDuplicateCheckKey] = useState(""); const [error, setError] = useState(""); const [dirty, setDirty] = useState(false); const [saving, setSaving] = useState(false); const [deleting, setDeleting] = useState(false); const { confirmDiscard } = useUnsavedChanges(dirty);
  ...
  function patch(field, value) { setDirty(true); if (field === "customerName" || field === "emirate") { setSimilar([]); setDuplicateCheckKey(""); } setValues((current) => ({ ...current, [field]: value })); }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (matches.length) {
        setSimilar(matches);
        setDuplicateCheckKey(getDuplicateCheckKey(values.customerName, values.emirate));
        setSaving(false);
        return;
      }
      await persist(false, []);
  ...
  return <div className="mx-auto max-w-4xl space-y-5">
    ...
    <form ref={formRef} onSubmit={handleSubmit} onInvalidCapture={handleInvalidCapture} className="space-y-5">
      ...
      <div className="flex flex-col-reverse justify-between gap-3 sm:flex-row">{editing ? <button type="button" onClick={handleDelete} disabled={deleting} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-800 px-4 py-2 text-red-300"><Trash2 size={17}/>{deleting ? "Deleting..." : "Delete customer"}</button> : <Link to={customersReturnTo} state={customerReturnState} onClick={(event) => { if (!confirmDiscard()) event.preventDefault(); }} className="rounded-xl border border-slate-700 px-4 py-2 text-center">Back to customers</Link>}<button type="submit" disabled={saving} className="rounded-xl bg-blue-600 px-4 py-2 font-medium disabled:opacity-60">{saving ? "Saving..." : "Save customer"}</button></div>
    </form>
    {similar.length > 0 && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"><div className="w-full max-w-2xl rounded-2xl border border-amber-700 bg-slate-900 p-6"><h2 className="text-xl font-semibold text-amber-200">A similar customer may already exist</h2><p className="mt-2 text-slate-400">Review the records below before creating another laboratory.</p><div className="mt-4 max-h-80 space-y-3 overflow-y-auto">{similar.map((candidate) => <article key={candidate.customer_id} className="rounded-xl border border-slate-700 bg-slate-950 p-4"><div className="flex flex-wrap justify-between gap-3"><div><h3 className="font-semibold">{candidate.customer_name}</h3><p className="text-sm text-slate-400">{candidate.emirate || "Emirate not recorded"}</p><p className="mt-2 text-sm text-amber-300">{candidate.match_reason}</p></div><Link target="_blank" to={`/customers/${candidate.customer_id}/overview`} className="self-start rounded-lg border border-blue-800 px-3 py-2 text-sm text-blue-300">View existing</Link></div></article>)}</div><div className="mt-5 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setSimilar([])} className="rounded-xl border border-slate-700 px-4 py-2">Go back and edit</button><button type="button" disabled={saving || !canOverrideSimilarCustomer({ candidates: similar, checkedKey: duplicateCheckKey, currentKey: getDuplicateCheckKey(values.customerName, values.emirate) })} onClick={() => persist(true, similar)} className="rounded-xl bg-amber-600 px-4 py-2 font-medium text-black">Create customer anyway</button></div></div></div>}
  </div>;
}
