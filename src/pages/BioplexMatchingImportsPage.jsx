...
export default function BioplexMatchingImportsPage() {
  ...
  const [view, setView] = useState("New additions");
  ...
  const visibleItems = useMemo(() => {
    ...
    const source = view === "Materials" ? model.materials : view === "Relationships" ? model.relationships : model.materials.filter((item) => item.classification === "New material");
  ...
  async function review() {
    ...
    try {
      ...
      const classifiedRows = classifyParsedImportRows(nextParsed.rows, nextSnapshot);
      setParsed({ ...nextParsed, rows: classifiedRows });
  ...
  return <div className="mx-auto max-w-7xl space-y-5">
    ...
    {parsed && <>
      ...
      <section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900 p-5">
        <div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => setView("New additions")} className={`rounded-xl px-4 py-2 ${view === "New additions" ? "bg-emerald-600" : "border border-slate-700"}`}>New additions ({model.materials.filter((item) => item.classification === "New material").length})</button><button type="button" onClick={() => setView("Relationships")} className={`rounded-xl px-4 py-2 ${view === "Relationships" ? "bg-blue-600" : "border border-slate-700"}`}>Relationships ({model.relationships.length})</button><button type="button" onClick={() => setView("Materials")} className={`rounded-xl px-4 py-2 ${view === "Materials" ? "bg-blue-600" : "border border-slate-700"}`}>Materials ({model.materials.length})</button><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search assay or lot" className={`${inputClass} ml-auto max-w-sm`}/></div>
