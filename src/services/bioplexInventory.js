...
import {
  BIOPLEX_COUNT_CONFLICT_MESSAGE,
  bioplexConflictError,
  expectedBioplexTimestamp,
} from "../utils/bioplexConcurrency";
...
function itemPayload(item,index){const blank=item.quantity===""||item.quantity===null||item.quantity===undefined;const quantity=blank?null:Number(item.quantity);if(quantity!==null&&(!Number.isInteger(quantity)||quantity<0))throw new Error("Quantities must be whole numbers of zero or greater.");return{clientKey:item.clientKey,assayId:item.assayId||null,productId:item.productId||null,referenceLotId:item.referenceLotId||null,materialType:item.materialType,lotNumber:String(item.lotNumber??"").trim(),quantity,expiryDate:item.expiryDate||null,verificationStatus:item.verificationStatus==="Manual"?"Manually Entered":item.verificationStatus||"Matched",assayName:String(item.assayName??"").trim(),productCode:String(item.productCode??"").trim()||null,productName:String(item.productName??"").trim(),matchingImportId:item.matchingImportId||null,notes:String(item.notes??"").trim()||null,displayOrder:index};}
export async function saveBioplexCount(payload){const {data,error}=await supabase.rpc("save_bioplex_count",{p_count_id:payload.countId?Number(payload.countId):null,p_customer_id:Number(payload.customerId),p_counted_on:payload.countedOn,p_notes:String(payload.notes??""),p_status:payload.status,p_items:payload.items.filter((item)=>item.materialType!=="consumable"||!(item.quantity===""||item.quantity===null||item.quantity===undefined)).map(itemPayload),p_links:payload.links??[],p_correction_reason:String(payload.correctionReason??"")||null,p_expected_updated_at:expectedBioplexTimestamp(payload.expectedUpdatedAt,"BioPlex count",Boolean(payload.countId))});if(error)throw bioplexConflictError(error,BIOPLEX_COUNT_CONFLICT_MESSAGE);return Number(data);}
