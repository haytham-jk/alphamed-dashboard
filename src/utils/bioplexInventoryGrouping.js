...
function compareItems(first, second) {
  return orderValue(first) - orderValue(second) ||
    String(first?.assay_name_snapshot ?? first?.assayName ?? "").localeCompare(String(second?.assay_name_snapshot ?? second?.assayName ?? "")) ||
    String(first?.product_name_snapshot ?? first?.productName ?? "").localeCompare(String(second?.product_name_snapshot ?? second?.productName ?? "")) ||
    String(first?.lot_number ?? first?.lotNumber ?? "").localeCompare(String(second?.lot_number ?? second?.lotNumber ?? ""));
}
function itemId(item) { return item?.id ?? item?.clientKey; }
function parentId(link) { return link?.parent_item_id ?? link?.parentKey; }
function childId(link) { return link?.child_item_id ?? link?.childKey; }
export function buildBioplexInventoryGroups(items = [], links = []) {
  ...
  for (const link of links) {
    const parent = parentId(link); const child = childId(link);
    if (!byId.has(parent) || !byId.has(child)) continue;
    const list = parentLinks.get(parent) ?? [];
    list.push(link); parentLinks.set(parent, list);
    childParentCount.set(child, (childParentCount.get(child) ?? 0) + 1);
  }
  const reagentGroups = items.filter((item) => item.material_type === "kit" || item.materialType === "kit").sort(compareItems).map((root) => {
    const children = (parentLinks.get(itemId(root)) ?? []).map((link) => ({ item: byId.get(childId(link)), relationshipType: link.relationship_type ?? link.relationshipType, shared: (childParentCount.get(childId(link)) ?? 0) > 1 })).filter((entry) => entry.item).sort((first, second) => {
  ...
  const standalone = items.filter((item) => { const type = item.material_type ?? item.materialType; return type !== "kit" && !linkedChildren.has(itemId(item)); }).sort(compareItems);
